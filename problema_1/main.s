...
@ 
    LDR r0, =devmem
    LDR r1, =accessFlags @ =(O_RDWR + O_SYNC)
    MOV r7, #sys_open
    SVC 0
@
@@ End open file
...
@@ Enable the UART.
@               
    MOV r0, #UART_UARTEN_OFF @ disable uart to begin changing settings
    STR r0,[r1, #UART_CR]    @ storing the value 0 to the first bit position 
...
fifo_status: @ looping until the there is no data i/o
    ...
    ANDS r0, #UART_RXFE @ 'AND and Set Flags' operation to check if the bit on the position of 
                        @ the receive FIFO is empty == 1, if its not branch back
                        @ to check again until the bit is == 1                     
    @ CMP r0, #0
    @ CBNZ r0, fifo_status
    BNE fifo_status
...
@ Enable UART again
    MOV r0, #UART_UARTEN_ON @ enabling uart 
    STR r0,[r1, #UART_CR]  

...
@ mem protection options
    ...
    @ Data reception
waitRx:    ldr	r1, [r8, #UART_FR]
        and	r1, #0b10000 @ 	RXFE
        cmp	r1, #0
        BNE waitRx		@ equal to zero means that Rx FIFO contains bytes


    ldr r1, [r8, #UART_DR]		@ Load the message from UART Data Register
@    MOV r2, #0b11111111
@    BIC r1, r2

    MOVS r0, r1 @ coloca valor em r0 para devolver ao c
    BX LR
